...
namespace Search_and_SortDataAnalyser
{
    ...
    {
        private List<string> _contents_256 = new List<string> { Program.S_1_256_Contents, Program.S_2_256_Contents, Program.S_3_256_Contents };
        private List<string> _contents_2048 = new List<string> { Program.S_1_2048_Contents, Program.S_2_2048_Contents, Program.S_3_2048_Contents };

        public Initialise()
        {
            for (int i = 0; i < _contents_256.Count; i++)  // Iterates 3 times (for each 256 data file)
            {
            ...
            for (int i = 0; i < _contents_2048.Count; i++)  // Iterates 3 times (for each 2048 data file)
            {
        ...
        public int[] GetArray(string dataPoints, int index)
        {
            ...
            if (dataPoints == "256")
            {
                currrentContents = _contents_256[index];
            }
            else if (dataPoints == "2048")
            {
                currrentContents = _contents_2048[index];
            }
